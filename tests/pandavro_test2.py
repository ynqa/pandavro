...
def test_schema_infer(dataframe, schema_expected):
    assert schema_expected == pdx.schema_infer(dataframe, times_as_micros=True)


def test_schema_infer_times_as_millis(dataframe, schema_expected):
    schema_expected["fields"][1] = {'type': ['null', {'logicalType': 'timestamp-millis', 'type': 'long'}],
                    'name': 'DateTime64'},
    assert schema_expected == pdx.schema_infer(dataframe, times_as_micros=False)


def test_schema_infer_complex_types(dataframe, schema_expected):

    schema_expected["fields"].extend(
        [
            {'type': ['null', {
                'fields':
                    [
...
def test_fields_infer(dataframe, schema_expected):
    assert schema_expected["fields"] == pdx.__fields_infer(dataframe, nested_record_names={})
